...
for i in item_set:
    ...
    for k in item_set:
        ...
        for j in bin_set:
            ...
            # if i != k:
            # constraint 14
            # model.addConstr(R[i][4] + R[k][5], gb.GRB.LESS_EQUAL, p[i, j] + p[k, j] - 1)

for i in item_set:
    # constraint 12
    model.addConstr(gb.quicksum(gb.quicksum(beta[i, j, k] + 2 * g[i] for j in bin_set) for k in [1,2]), gb.GRB.EQUAL, 2)
